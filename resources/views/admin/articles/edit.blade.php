...
<div class="container-fluid py-4">
    ...
    <form action="{{ route('admin.articles.update', $article) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <textarea name="excerpt" id="excerpt" class="form-control" rows="2">{{ old('excerpt', $article->excerpt) }}</textarea>
            @error('excerpt')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="image_path" id="image_path" class="form-control" value="{{ old('image_path', $article->image_path) }}">
            @error('image_path')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
